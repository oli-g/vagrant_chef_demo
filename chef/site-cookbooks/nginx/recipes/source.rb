...
node.set[:nginx][:configure_flags] = [
  ...
  "--http-client-body-temp-path=#{node[:nginx][:temp_dir]}/client",
...
directory node[:nginx][:temp_dir] do
  owner "root"
  group "root"
  mode 0755
end
...
if node[:nginx][:status_page]
  cookbook_file "#{node[:nginx][:dir]}/conf.d/status.conf" do
    source "status.conf"
    owner "root"
    group "root"
    mode "0644"
